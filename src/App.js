...
function App() {
  ...
  const addToCart = async (product, variant, variantType, qty = 1) => {
    ...
    const data = { userId: user._id, productId: product._id, quantity: qty, variant, variantType };
    const res = await postData("/api/cart/add", data);
    if (!res.error) {
      setAlertBox({ open: true, error: false, msg: "Product added to cart!" });
    }
  };
          ...
          <div className="main d-flex">
            {isHeaderSidebarShow && (
              <>
                ...
                {windowWidth < 992 && isToggleSidebar && (
                  <div
                    className="sidebarOverlay active"
                ...
                <div className={`sidebarWrapper ${
                  windowWidth >= 992
                    ? (isToggleSidebar ? "toggle" : "")           // desktop: toggle hides it
                    : (isToggleSidebar ? "open" : "")             // mobile:  toggle shows it
                }`}>
                  <Sidebar />
                </div>
              </>
            ...
            <div className={`content ${isToggleSidebar && windowWidth >= 992 ? "toggle" : ""}`}>
              {authChecked && (
                <Routes>
